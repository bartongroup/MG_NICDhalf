...
@tbl-half-lin shows the half life time for each condition, derived from the linear fits, together with propagated standard errors.

```{r}
#| label: tbl-half-lin
#| tbl-cap: Half life times from fitting linear regression to time points 0, 1, 2, 4^h^. Errors are standard errors.
tar_read(lin) |> 
  mutate(
    `Half life (h)` = paste0(
      formatC(signif(t_half, digits = 2)),
      "±",
      formatC(signif(std_t_half, digits = 1))
    )
  ) |> 
  select(condition, `Half life (h)`) |> 
  gt()
```

## Exponential model with background
...
Here $b$ represents a constant background, perhaps due to LY treatment not being perfect.

We fit this model to the original normalised data (without log-transformation), to all time points. The best fitting curves are shown in @fig-dat-exp and the best-fitting parameters, including the derived half-live are shown in @tbl-half-exp.
